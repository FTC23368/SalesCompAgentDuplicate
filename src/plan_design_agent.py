...
class PlanDesignAgent:
    ...
    def generate_response(self, retrieved_content: List[str], messageHistory: list[BaseMessage]) -> str:
        plan_design_prompt = get_prompt("planexplainer").format(retrieved_content=retrieved_content)
        llm_messages = create_llm_msg(plan_design_prompt, messageHistory)
        return self.model.stream(llm_messages)

    def plan_design_agent(self, state: dict) -> dict:
        retrieved_content = self.retrieve_documents(state['initialMessage'])
        return {
            "lnode": "plan_design_agent", 
