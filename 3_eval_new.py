...
def test_formatting():
    ...
    for idx, result in enumerate(results):
        st.markdown(f"# Eval: {idx}")
        st.text(f"input: {result['input']}")
        st.text(f"expected: {result['expected']}")
        st.text(f"actual: {result['actual']}")
        st.text(f"category match: {result['cat_result']}")
        st.text(f"result match: {result['resp_result']}")
        st.text(f"response: {result['resp_reason']}")
        st.divider()
